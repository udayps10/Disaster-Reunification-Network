        ...
        builder: (context, state, child) {
          return MaterialApp(
            ...
            routes: {
              ...
              '/login_normal': (context) => const LoginScreen(role: UserRole.user),
              '/login_official': (context) => const LoginScreen(role: UserRole.official),
              ...
              '/add_normal': (context) => const AddRecordScreen(type: RecordType.normal),
              '/add_critical': (context) => const AddRecordScreen(type: RecordType.critical),
            ...
            builder: (context, child) {
              WidgetsBinding.instance.addPostFrameCallback((_) {
                ...
                if (state.status == AuthStatus.unauthenticated) {
                  ...
                  final publicRoutes = ['/welcome', '/login_normal', '/login_official', '/register', '/forgot_password', '/'];
                ...
                if (state.status == AuthStatus.authenticatedUnverified) {
                  if (currentRoute != '/verify_email') {
                    navigator.pushNamedAndRemoveUntil('/verify_email', (route) => false);
                  }
                  return;
                }
                ...
                if (state.status == AuthStatus.authenticatedOfficialPending) {
                  if (currentRoute != '/pending_approval') {
                    navigator.pushNamedAndRemoveUntil('/pending_approval', (route) => false);
                  }
                  return;
                }
                ...
                if (state.status == AuthStatus.profileError) {
                  if (currentRoute != '/profile_error') {
                    navigator.pushNamedAndRemoveUntil('/profile_error', (route) => false);
                  }
                  return;
                }

                // If authenticated and verified, redirect away from auth screens
                final authRoutes = ['/welcome', '/login_normal', '/login_official', '/register', '/forgot_password', '/', '/verify_email', '/pending_approval', '/profile_error'];
                if (authRoutes.contains(currentRoute)) {
                  if (state.status == AuthStatus.authenticatedVerified) {
                    navigator.pushNamedAndRemoveUntil('/user_home', (route) => false);
                  } else if (state.status == AuthStatus.authenticatedOfficial) {
                    navigator.pushNamedAndRemoveUntil('/official_dashboard', (route) => false);
                  }
                }
              });
              return child!;
